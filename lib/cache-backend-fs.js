...
var FSBackend = function FSBackend(loadParameter) {
  ...
  this.index = [];
  this.location =
    typeof loadParameter === "string" && loadParameter.length > 0
      ? loadParameter
      : process.cwd() + "/cache/";
  this.location =
    this.location.substr(this.location.length - 1) === "/"
      ? this.location
      : this.location + "/";
};
...
function sanitisePath(path, queueObject) {
  ...
  var sanitisedPath = path.length ? path.replace(/\s*$/gi, "") : "index.html";
  var headers = queueObject.stateData.headers;
  var sanitisedPathParts;
  ...
  if (sanitisedPath.match(/\?/)) {
    ...
    var hashedQS = crypto
      .createHash("sha1")
      .update(sanitisedPathParts.join("?"))
      .digest("hex");
  ...
  if (
    !sanitisedPath.match(/\.[a-z0-9]{1,6}$/i) ||
    (headers["content-type"] &&
      headers["content-type"].match(/text\/html/i) &&
      !sanitisedPath.match(/\.htm[l]?$/i))
  ) {
    var subMimeType = "";
    var mimeParts = [];
    ...
    if (
      ...
      headers["content-type"] &&
      (mimeParts = headers["content-type"].match(
...
FSBackend.prototype.load = function() {
  ...
  if (
    !backend.fileExists(backend.location) &&
    backend.isDirectory(backend.location)
  ) {
  ...
  try {
    var fileData;
    if (
      (fileData = fs.readFileSync(backend.location + "cacheindex.json")) &&
      fileData.length
    ) {
...
FSBackend.prototype.setItem = function(queueObject, data, callback) {
  ...
  pathStack = pathStack.concat(
    sanitisePath(queueObject.path, queueObject).split(/\/+/g)
  );

  var cacheItemExists = false;
  var firstInstanceIndex = NaN;
  if (
    backend.index.reduce(function(prev, current, index) {
      firstInstanceIndex = !isNaN(firstInstanceIndex)
        ? firstInstanceIndex
        : index;
  ...
  pathStack.forEach(function(pathChunk, count) {
    var currentPath =
      backend.location + pathStack.slice(0, count + 1).join("/");
    if (
      backend.fileExists(
        backend.location + pathStack.slice(0, count + 1).join("/")
      )
      ...
      if (!backend.isDirectory(currentPath)) {
        if (count === pathStack.length - 1) {
          ...
          throw new Error(
            "Cache storage of resource (%s) blocked by file: %s",
            queueObject.url,
            currentPath
          );
        }
