...
Cookie.fromString = function(string) {
  ...
  function parseKeyVal(input) {
    ...
    var val = input.split(/=/).slice(1).join("=");
  ...
  var name = parseKeyVal(parts.shift());
  var keyValParts = {};
  ...
  parts
    .filter(function(input) {
      return Boolean(input.replace(/\s+/gi, "").length);
    })
    .map(parseKeyVal)
    .forEach(function(keyval) {
      var key = String(keyval[0]).toLowerCase().replace(/[^a-z0-9]/gi, "");
...
Cookie.prototype.matchDomain = function(domain) {
  ...
  var reverseDomainComp = domain.split("").reverse().join("");
