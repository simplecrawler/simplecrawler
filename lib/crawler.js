...
Crawler.prototype.fetchQueueItem = function(queueItem) {
  ...
  if(crawler.supportCookies) {
    requestOptions.headers.cookie = crawler._cookies.join(';');
  }
    ...
    // Check and see if we need to set our cookie property
    if(crawler.supportCookies && response.headers.hasOwnProperty('set-cookie')) {
      crawler._cookies = _.union(crawler._cookies, response.headers['set-cookie']);
    }
