...
Crawler.prototype.start = function() {
    ...
    crawler.getRobotsTxt(function (error, robotsTxtUrl, robotsTxtBody) {
        if (error) {
            crawler.emit("robotstxterror", error);
        } else if (crawler.respectRobotsTxt) {
            var robots = robotsTxtParser(robotsTxtUrl, robotsTxtBody);

            crawler.urlIsAllowed = function (parsedURL) {
                parsedURL.hostname = parsedURL.host;

                var formattedURL = uri(parsedURL).href(),
                    allowed = false;

                // The punycode module sometimes chokes on really weird domain
                // names. Catching those errors to prevent crawler from crashing
                try {
                    allowed = robots.isAllowed(formattedURL, crawler.userAgent);
                } catch (error) {
                    // URL will be avoided
                }
...
Crawler.prototype.getRequestOptions = function (queueItem) {
    ...
    if (typeof queueItem === "string") {
        queueItem = crawler.processURL(queueItem);
    }
...
*/
Crawler.prototype.getRobotsTxt = function (callback) {
    var crawler = this;

    var client = crawler.initialProtocol === "https" ? https : http;
    ...
    var requestOptions = crawler.getRequestOptions(robotsTxtUrl);

    // Get the resource!
    var clientRequest = client.request(requestOptions, function(response) {
        if (response.statusCode >= 200 && response.statusCode < 300) {
            var responseLength = parseInt(response.headers["content-length"], 10) || crawler.maxResourceSize,
            ...
            response.on("end", function () {
                callback(undefined, robotsTxtUrl, responseBuffer.toString());
            });
        } else {
            response.socket.destroy();

            var errorMsg = util.format("Server responded with status %d when fetching robots.txt", response.statusCode);
