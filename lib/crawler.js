...
const FetchQueue = require("./queue.js");
const CookieJar = require("./cookies.js");
...
const robotsTxtParser = require("robots-parser");
...
var Crawler = function(initialURL) {
  ...
  var crawler = this;
  var parsedURL = uri(initialURL).normalize();
  ...
  this.discoverRegex = [
    ...
    function(string) {
      ...
      return Array.isArray(result)
        ? String(result[2]).split(",").map(function(string) {
...
Crawler.prototype.getRequestOptions = function(queueItem) {
  ...
  var requestPort = queueItem.port;
  var requestPath = queueItem.path;
  ...
  var isStandardHTTPSPort =
    queueItem.protocol === "https" && queueItem.port !== 443;
  var isStandardPort = isStandardHTTPPort || isStandardHTTPSPort;
  ...
  if (crawler.acceptCookies && crawler.cookies.getAsHeader()) {
    requestOptions.headers.cookie = crawler.cookies.getAsHeader(
      queueItem.host,
      queueItem.path
    );
  }
...
Crawler.prototype.getRobotsTxt = function(url, callback) {
  ...
  var clientRequest = client.request(requestOptions, function(response) {
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      var responseBuffer = new Buffer(responseLength);
      var responseLengthReceived = 0;
      ...
      var decodeAndReturnResponse = function(error, responseBuffer) {
        ...
        var responseBody = crawler.decodeBuffer(responseBuffer, contentType);
...
Crawler.prototype.cleanExpandResources = function(urlMatch, queueItem) {
  ...
  for (let i = 0; i < urlMatch.length; i++) {
    ...
    try {
      URL = uri(URL).absoluteTo(queueItem.url || "").normalize().href();
    } catch (e) {
...
Crawler.prototype.discoverResources = function(resourceText) {
  ...
  if (
    ...
    /<meta(?:\s[^>]*)?\sname\s*=\s*["']?robots["']?[^>]*>/i.test(resourceText)
  ) {
    var robotsValue = /<meta(?:\s[^>]*)?\svalue\s*=\s*["']?([\w\s,]+)["']?[^>]*>/i.exec(
...
Crawler.prototype.domainValid = function(host) {
  ...
  return (
    ...
    (crawler.ignoreWWWDomain &&
      crawler.host.replace(/^www\./i, "") === host.replace(/^www\./i, "")) ||
...
Crawler.prototype.handleResponse = function(
  ...
  var timeHeadersReceived = Date.now();
  var timeDataReceived;
  var redirectQueueItem;
  var responseBuffer;
  var responseLength;
  var responseLengthReceived = 0;
  var contentType = response.headers["content-type"];
            ...
            if (
              ...
              zlib.unzip(responseBuffer, function(error, decompressedBuffer) {
                if (error) {
                  ...
                  var responseBody = crawler.decompressResponses
                    ? decompressedBuffer
                    : responseBuffer;
  ...
  crawler.queue.oldestUnfetchedItem(function(error, queueItem) {
    if (error) {
      ...
      if (
        ...
        crawler.getRobotsTxt(robotsTxtUrl, function(
          error,
          robotsTxtUrl,
          robotsTxtBody
        ) {
          if (error) {
            ...
            crawler._robotsTxts.push(
              robotsTxtParser(robotsTxtUrl, robotsTxtBody)
            );
          }
