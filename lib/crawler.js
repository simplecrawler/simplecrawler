...
/*
    ...
    Examples
        ...
        crawler.decodeResponse(responseBuffer, "text/html; charset=tis-620");

    Returns the decoded buffer.

*/
Crawler.prototype.decodeResponse = function (buffer, contentTypeHeader) {
    contentTypeHeader = contentTypeHeader || "";

    var embeddedEncoding = /<meta.*charset=(?:"|'){0,1}([^"|']*)[^>]/i.exec(buffer.toString(undefined, 0, 512)) || [],
    ...
    function processReceivedData() {
        ...
        if (crawler.depthAllowed(queueItem)) {
            if (crawler.decodeResponses) {
                decodedResponse = crawler.decodeResponse(responseBuffer, response.headers["content-type"]);
            }
