...
describe("Crawler link discovery", function() {
  ...
  it("should replace all '&amp;'s with ampersands", function() {
    var links = discover(
      "<a href='http://example.com/resource?with&amp;query=params&amp;and=entities'>"
    );

    links.should.be.an("array");
    links.length.should.equal(2);
    links[0].should.equal(
      "http://example.com/resource?with&query=params&and=entities"
    );
    links[1].should.equal("http://example.com/resource");
  });
  ...
  it("should find and follow meta redirects", function() {
    var links = discover(
      "<meta http-equiv='refresh' content='0; url=/my/other/page.html'>",
      {
  ...
  it("should ignore HTML comments with parseHTMLComments = false", function() {
    ...
    var links = discover(
      "  <!-- http://example.com/oneline_comment --> " +
        " <a href=google.com> " +
        " <!-- " +
  ...
  it("should ignore script tags with parseScriptTags = false", function() {
    ...
    var links = discover(
      "  <script>var a = \"<a href='http://example.com/oneline_script'></a>\";</script> " +
        " <a href=google.com> " +
  ...
  it("should discover URLs legitimately ending with a quote or parenthesis", function() {
    var links = discover(
      "<a href='example.com/resource?with(parentheses)'>" +
  ...
  it("should not pick up 'href' or 'src' inside href attributes as full URL's", function() {
    var links = discover(
      "<a href='https://example.com/?src=3'>My web page</a>"
    );
  ...
  it("should find resources in srcset attributes", function() {
    var links = discover(
      "<img src='pic.png' srcset='pic-200.png, pic-400.png 400w, pic-800.png 2x'>",
      {
        url: "https://example.com/"
      }
    );
  ...
  it("should respect nofollow values in robots meta tags", function() {
    discover(
      "<meta name='robots' value='nofollow'><a href='/stage2'>Don't follow me!</a>"
    ).should.eql([]);

    discover(
      "<meta name='robots' value='nofollow, noindex'><a href='/stage2'>Don't follow me!</a>"
    ).should.eql([]);
  });
});
