...
describe("Crawler reliability", function() {
  ...
  it("should be able to be started, then stopped, then started again", function(
    done
  ) {
  ...
  it("should not decrement _openRequests below zero in the event of a timeout", function(
    done
  ) {
    var localCrawler = new Crawler("http://127.0.0.1:3000/timeout");
    localCrawler.timeout = 200;
  ...
  it("should decrement _openRequests in the event of a non-supported mimetype", function(
    done
  ) {
  ...
  it("should add multiple items with the same URL to the queue if forced to", function(
    done
  ) {
    var localCrawler = makeCrawler("http://127.0.0.1:3000/");
    var addCount = 0;
  ...
  it("should be able to freeze and defrost the queue", function(done) {
    var localCrawler = makeCrawler("http://127.0.0.1:3000/");
    var newCrawler = makeCrawler("http://127.0.0.1:3000/");
    var tmp = os.tmpdir()
      ? path.join(os.tmpdir(), "queue.json")
    ...
    function testQueue(crawler, oldestUnfetchedIndex) {
      ...
      crawler.queue._scanIndex["http://127.0.0.1:3000/stage2"].should.equal(
        true
      );
      crawler.queue._scanIndex["http://127.0.0.1:3000/stage/3"].should.equal(
        true
      );
      crawler.queue._scanIndex["http://127.0.0.1:3000/stage/4"].should.equal(
        true
      );
  ...
  describe("when stopping the crawler", function() {
    ...
    it("should terminate open connections when requested", function(done) {
      ...
      localCrawler.on("fetchstart", function() {
        ...
        localCrawler._openRequests.length.should.equal(
          2,
