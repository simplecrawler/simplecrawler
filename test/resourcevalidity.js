...
/**
 * @file Tests whether a given resource is considered 'valid' for crawling under
...
describe("Resource validity checker", function() {
  ...
  it("should strip WWW from processed URL's altogether", function() {
    ...
    crawler
      .processURL("http://www.example.com")
    ...
    crawler
      .processURL("http://www.example.com")
      .host.should.equal("www.example.com");
  });
  ...
  it("should strip query strings from processed URL's", function() {
    ...
    crawler
      .processURL("http://example.com/example?q=crawler")
      .path.should.equal("/example");
    crawler
      .processURL("http://example.com/test?q=crawler&foo=bar")
      .path.should.equal("/test");

    crawler.stripQuerystring = false;
    crawler
      .processURL("http://example.com/example?q=crawler")
      .path.should.equal("/example?q=crawler");
    crawler
      .processURL("http://example.com/test?q=crawler&foo=bar")
      .path.should.equal("/test?q=crawler&foo=bar");
  });
  ...
  it("should canonicalize query strings by sorting parameters", function() {
    ...
    crawler
      .processURL("http://example.com/example?s=1&r=9&b=3&r=2&r=7")
      .path.should.equal("/example?b=3&r=9&r=2&r=7&s=1");
    crawler
      .processURL("http://example.com/test?q=crawler&foo=bar")
      .path.should.equal("/test?foo=bar&q=crawler");

    crawler.sortQueryParameters = false;
    crawler
      .processURL("http://example.com/example?s=1&r=9&b=3&r=2&r=7")
      .path.should.equal("/example?s=1&r=9&r=2&r=7&b=3");
  ...
  it("should throw out junky or invalid URLs without dying", function() {
    ...
    crawler
      .processURL("ur34nfie4985:s////dsf/", urlContext)
      .should.equal(false);
  });
  ...
  it("should decode responses based on Content-Type headers", function(done) {
    decodingTest("/encoded/header", function(queueItem, responseBody) {
      responseBody
        .trim()
        .should.equal("Eyjafjallajökull er fimmti stærsti jökull Íslands.");
      done();
    });
  });

  it("should decode responses based on inline charset definitions", function(
    done
  ) {
    decodingTest("/encoded/inline", function(queueItem, responseBody) {
      responseBody
        .trim()
        .should.equal(
          "<meta charset=\"iso-8859-1\"><p>Pippi Långstrump är en av Astrid Lindgrens mest kända litterära figurer.<p>"
        );
      done();
    });
  });

  it("should decode responses based on older inline charset definitions", function(
    done
  ) {
    decodingTest("/encoded/old-inline", function(queueItem, responseBody) {
      responseBody
        .trim()
        .should.equal(
          "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=iso-8859-1\" /><p>Preikestolen er et fjellplatå på nordsiden av Lysefjorden i Forsand.<p>"
        );
      done();
    });
  });
  ...
  it("should decompress gzipped responses by default", function(done) {
    ...
    crawler.on("fetchcomplete", function(queueItem, responseBody) {
      responseBody
        .toString()
        .should.equal("Yay, you know how to deal with gzip compression!");
      done();
    });
    crawler.start();
  });
  ...
  it("should decompress deflated responses by default", function(done) {
    ...
    crawler.on("fetchcomplete", function(queueItem, responseBody) {
      responseBody
        .toString()
        .should.equal("Yay, you know how to deal with deflate compression!");
      done();
    });
    crawler.start();
  });

  it("should be able to not decompress responses (but still find inline resources)", function(
    done
  ) {
    ...
    crawler.on("fetchcomplete", function(queueItem, responseBody) {
      ...
      var body =
        queueItem.path === "/compressed/link"
