...
/**
 * @file Runs a very simple crawl on an HTTP server
...
describe("Test Crawl", function() {
  ...
  it("should be able to disregard rules in robots.txt", function(done) {
    ...
    crawler.on("complete", function() {
        ...
        new Error("Didn't visit forbidden URL (even though it should have)")
      );
    });
  });
  ...
  it("should emit an error when robots.txt redirects to a disallowed domain", function(
    done
  ) {
    var server = new Server({
  ...
  it("should support async event listeners for manual discovery", function(
    done
  ) {
    var crawler = makeCrawler("http://127.0.0.1:3000/");
  ...
  it("should not throw an error if header Referer is undefined", function(
    done
  ) {
  ...
  it("it should remove script tags if parseScriptTags is disabled", function(
    done
  ) {
  ...
  it("should allow initial redirect to different domain if configured", function(
    done
  ) {
  ...
  it("should only allow redirect to different domain for initial request", function(
    done
  ) {
  ...
  it("should disallow initial redirect to different domain by default", function(
    done
  ) {
    var crawler = makeCrawler("http://127.0.0.1:3000/domain-redirect");
    var linksDiscovered = 0;
  ...
  it("should not increase depth on multiple redirects on the initial request", function(
    done
  ) {
  ...
  it("should disallow initial redirect to different domain after a 2xx", function(
    done
  ) {
    var crawler = makeCrawler("http://127.0.0.1:3000/to-domain-redirect");
    var discoComplete = 0;
